...
class NFoxHeadModel(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            NFoxHead_num_heads (int, optional): Number of additional tokens to predict. Defaults to 3.
            NFoxHead_num_layers (int, optional): Number of ResBlock layers for each NFoxHead head. Defaults to 0.
        """
        super().__init__()
        ...
        # Ensure NFoxHead_head's dtype and device align with the base_model
        self.NFoxHead_head.to(self.base_model.dtype).to(self.base_model.device)
    ...
    def from_pretrained(
        cls,
        NFoxHead_head_name_or_path,
        base_model=None,
        NFoxHead_num_heads=None,
        **kwargs,
    ):
        """
        Args:
            NFoxHead_head_name_or_path (str): Name or path of the NFoxHead head to load.
            **kwargs: Additional keyword arguments for loading the base model.

        Returns:
            NFoxHeadModel: A NFoxHeadModel instance loaded from the given path.
        """
        NFoxHead_config = NFoxHeadConfig.from_pretrained(NFoxHead_head_name_or_path)
        if NFoxHead_num_heads is not None:
            print("Overriding NFoxHead_num_heads as:", NFoxHead_num_heads)
            NFoxHead_config.NFoxHead_num_heads = NFoxHead_num_heads
        if base_model is not None:
            print("Overriding base_model as:", base_model)
            NFoxHead_config.base_model_name_or_path = base_model
            
        base_model = KVLlamaForCausalLM.from_pretrained(
            NFoxHead_config.base_model_name_or_path, **kwargs
        )

        model = cls(
            base_model,
            NFoxHead_config.NFoxHead_num_heads,
            NFoxHead_config.NFoxHead_num_layers,
            NFoxHead_config.base_model_name_or_path,
        )
        NFoxHead_head_path = os.path.join(NFoxHead_head_name_or_path, "NFoxHead_lm_head.pt")
        if os.path.exists(NFoxHead_head_path):
            filename = NFoxHead_head_path
        else:
            filename = hf_hub_download(NFoxHead_head_name_or_path, "NFoxHead_lm_head.pt")
        NFoxHead_head_state_dict = torch.load(filename, map_location=base_model.device)
    ...
    def forward(
        ...
        for i in range(self.NFoxHead):
            mhidden_states = self.NFoxHead_head[i](hidden_states)
            mlogits = self.base_model.lm_head(mhidden_states)
            NFoxHead_logits.append(mlogits)
        if output_orig:
