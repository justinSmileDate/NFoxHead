...
def load_model_function(model_name, load_in_8bit=False, load_in_4bit=False):
    model_name = model_name or "FasterDecoding/NFoxHead-vicuna-7b-v1.3"
...
def NFoxHead_chat_interface(user_input, temperature, max_steps, no_history):
    ...
    outputs = model.NFoxHead_generate(input_ids, temperature=temperature, max_steps=max_steps)
    response = ""
...
if __name__ == "__main__":
    load_model_interface = gr.Interface(
        load_model_function,
        [
            gr.components.Textbox(placeholder="FasterDecoding/NFoxHead-vicuna-7b-v1.3", label="Model Name"),
        ...
        description="Load NFoxHead Model",
        title="NFoxHead Model Loader",
        live=False,
        api_name="load_model"
    )
    ...
    chat_interface = gr.Interface(
        NFoxHead_chat_interface,
        [
            gr.components.Textbox(placeholder="Ask NFoxHead...", label="User Input"),
        ...
        [
            gr.components.Textbox(label="NFoxHead's Response", type="text"),
            gr.components.Textbox(label="Chat History", type="text")
        ],
        live=False,
        description="Chat with NFoxHead",
        title="NFoxHead Chatbox",
        api_name="chat"
    )
