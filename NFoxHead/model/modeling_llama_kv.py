...
# 2. Modifying attention mask for integration with NFoxHead
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        ...
        # [MODIFIED] add NFoxHead mask
        if hasattr(self, "NFoxHead_mask") and self.NFoxHead_mask is not None:
            NFoxHead_mask = self.NFoxHead_mask
            NFoxHead_len = NFoxHead_mask.size(-1)
            combined_attention_mask[:, :, -NFoxHead_len:, -NFoxHead_len:][
                NFoxHead_mask == 0
            ] = combined_attention_mask.min()
            if hasattr(self, "NFoxHead_mode"):
                # debug mode
                if self.NFoxHead_mode == "debug":
                    torch.save(combined_attention_mask, "NFoxHead_mask.pt")
