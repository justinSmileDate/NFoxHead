...
import os
from NFoxHead.model.NFoxHead_model_legacy import NFoxHeadModel, NFoxHeadConfig

IGNORE_TOKEN_ID = LabelSmoother.ignore_index


# Customized for training NFoxHead heads
class CustomizedTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if hasattr(model, "module"):
            NFoxHead = model.module.NFoxHead
        else:
            NFoxHead = model.NFoxHead
        ...
        log = {}
        for i in range(NFoxHead):
            NFoxHead_logits = logits[i, :, : -(2 + i)].contiguous()
            NFoxHead_labels = labels[..., 2 + i :].contiguous()
            NFoxHead_logits = NFoxHead_logits.view(-1, logits.shape[-1])
            NFoxHead_labels = NFoxHead_labels.view(-1)
            NFoxHead_labels = NFoxHead_labels.to(NFoxHead_logits.device)
            loss_i = loss_fct(NFoxHead_logits, NFoxHead_labels)
            loss += loss_i
            not_ignore = NFoxHead_labels.ne(IGNORE_TOKEN_ID)
            NFoxHead_labels = NFoxHead_labels[not_ignore]

            # Add top-k accuracy
            for k in range(1, 2):
                _, topk = NFoxHead_logits.topk(k, dim=-1)
                topk = topk[not_ignore]
                correct = topk.eq(NFoxHead_labels.unsqueeze(-1)).any(-1)
                log[f"NFoxHead{i}_top{k}"] = correct.float().mean().item()

            log[f"NFoxHead{i}_loss"] = loss_i.item()
        self.log(log)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    NFoxHead_num_heads: int = field(
        default=1,
        metadata={"help": "Number of NFoxHead heads."},
    )
    NFoxHead_num_layers: int = field(
        default=1,
        metadata={"help": "Number of layers for each NFoxHead head."},
    )
...
def train():
    ...
    # Add NFoxHead heads
    NFoxHead_lm_head = NFoxHeadModel(
        model,
        NFoxHead_num_heads=training_args.NFoxHead_num_heads,
        NFoxHead_num_layers=training_args.NFoxHead_num_layers,
        base_model_name_or_path=model_args.model_name_or_path,
    )

    # Format output dir
    training_args.output_dir = f"{training_args.output_dir}_NFoxHead_mlp_{model_args.model_name_or_path.split('/')[-1]}_NFoxHead_{training_args.NFoxHead_num_heads}_lr_{training_args.learning_rate}_layers_{training_args.NFoxHead_num_layers}"


    # Load data
    data_module = make_supervised_data_module(tokenizer=tokenizer, data_args=data_args)

    # Generate NFoxHead config for pushing to HF hub
    NFoxHead_config = NFoxHeadConfig(
        NFoxHead_num_heads=training_args.NFoxHead_num_heads,
        NFoxHead_num_layers=training_args.NFoxHead_num_layers,
        base_model_name_or_path=model_args.model_name_or_path,
        version="2"
    ...
    # Save NFoxHead config
    NFoxHead_config.save_pretrained(training_args.output_dir)

    # Start trainner
    trainer = CustomizedTrainer(
        model=NFoxHead_lm_head, tokenizer=tokenizer, args=training_args, **data_module
    )
    ...
    # Save NFoxHeadHead seperately
    if hasattr(NFoxHead_lm_head, "module"):
        lm_head = NFoxHead_lm_head.module.NFoxHead_head
    else:
        lm_head = NFoxHead_lm_head.NFoxHead_head
    import deepspeed
    ...
    if local_rank == 0:
        ...
        save_file(
            state_dict,
            os.path.join(training_args.output_dir, "NFoxHead_lm_head.safetensors"),
        )
